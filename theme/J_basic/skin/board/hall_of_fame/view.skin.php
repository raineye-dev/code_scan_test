...
for ($i = 0; $i < count($view['file']); $i++) {
    if (!empty($view['file'][$i]['view'])) {
        // get_view_thumbnail() 결과에서 <a> 제거
        $img_html = get_view_thumbnail($view['file'][$i]['view']);
        $img_html = preg_replace('/<a[^>]*>(<img[^>]+>)<\/a>/i', '$1', $img_html);
        $images[] = $img_html;
    }
}
...
if (count($images)) {
?>
<div id="bo_v_img" class="view-slider">

    <div class="slides-wrapper">
        <?php foreach ($images as $img) { ?>
            <div class="slide-item"><?php echo $img; ?></div>
        <?php } ?>
    </div>
    ...
    <div class="dots-wrapper">
        <?php for ($d = 0; $d < count($images); $d++) { ?>
            <span class="dot" data-index="<?php echo $d; ?>"></span>
        <?php } ?>
    </div>
    <?php } ?>
...
document.addEventListener("DOMContentLoaded", function () {
    const slides = document.querySelectorAll("#bo_v_img .slide-item");
    const dots = document.querySelectorAll("#bo_v_img .dot");

    if (slides.length === 0) return;       // 이미지가 없으면 종료
    let index = 0;
  ...
  <div class="hall_of_fame_con_R">
		...
		<table class="table_t01">
			<caption>명예의전당 상세 내용입니다.</caption>
			<colgroup>
				<col width="140px">
				<col>
			</colgroup>								
			<tbody>
				...
				<tr>
					<th scope="row">년도</th>
					<td><?php if ($category_name) { ?><?php echo $view['ca_name']; // 분류 출력 끝 ?><?php } ?></td>
				</tr>
				<tr>
					<th scope="row">지역</th>
