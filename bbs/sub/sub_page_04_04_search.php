...
<?php
	...
	//*추가 sub_page_04_04에서 search값 search_con에 get방식으로 담아주기
	...
	//*where을 사용중이기 때문에 and 구절 추가
	$and = "AND grade>=70";
	
	//*where구절도 $where로 변경
	$where="WHERE idx = '$search_con' ";

	//*관리자가 입력한 값과 idx 값이 동일한 값을 모두 가져오도록 수정
	$total_res = sql_fetch("SELECT count(*) as cnt FROM video_education_info {$where} {$and} ");
	...
	//*search_con을 통해 관리자가 입력한 값과 idx 값이 동일한 데이터만 골라주도록 sql문 수정
	$sql = " select * from video_education_info {$where} {$and} order by idx desc limit $page_start, $page_size ";
...
<!-- *elseif문을 통해 idx와 search_con값이 같지 않을시 조건에 부합하는 게시물이 없습니다 출력 -->
<?}elseif($row['idx'] != $search_con){?>
		<tr>
...
<div class="btn_submits">
	<a href="/bbs/sub/sub_page_04_04.php" class="btn_submit_right">목록으로
  </div>
