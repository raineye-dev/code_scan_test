...
<td class="td_thumb" style="text-align:center; padding:4px;">
<?php
    // nw_content에서 첫 번째 img src 추출
    $thumb_url = '';
    if (!empty($row['nw_content'])) {
        if (preg_match('/<img[^>]+src=["\']([^"\']+)["\'][^>]*>/i', $row['nw_content'], $m)) {
            $thumb_url = $m[1];
        }
    }
    if ($thumb_url) {
        echo '<img src="'.htmlspecialchars($thumb_url).'" alt="미리보기"
                   style="width:60px; height:45px; object-fit:cover; border-radius:3px;
                          border:1px solid #ddd; cursor:pointer;"
                   onclick="window.open(this.src)" title="클릭하면 원본 이미지">';
    } else {
        echo '<span style="color:#bbb; font-size:11px;">없음</span>';
    }
?>
</td>
